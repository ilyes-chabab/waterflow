...
- [x] Les éventuelles étapes d'authentification et de renouvellement de l'authentification (expiration des jetons, etc.) sont intégrées correctement en suivant la documentation de l'API. *(clés API à durée de vie 90 jours, renouvellement en libre-service via `POST /api/renew-key`, banniere + bouton dans `ui.py`)*
...
- [x] Les adaptations d'interface nécessaires et en accord avec les spécifications sont intégrées à l'application. *(bandeau d'expiration + bouton de renouvellement dans la sidebar de `ui.py`)*
- [x] Les tests d'intégration couvrent tous les points de terminaison exploités. *(`tests/test_pipeline.py::test_expired_key_rejected`, `test_renew_key_extends_validity`)*
...
- [x] Le ou les outils pour l'intégration du monitorage du modèle sont adaptés au contexte et aux contraintes techniques du projet. *(Prometheus + Alertmanager + Grafana, cf. `alert_rules.yml`, `alertmanager.yml`)*
- [x] Des alertes sont configurées et déclenchées selon des seuils définis. *(`alert_rules.yml` : `APIDown`, `HighErrorRate`, `HighLatency`, routées vers Alertmanager)*
...
- [x] Les tests sont intégrés et respectent la couverture souhaitée établie. *(seuil `fail_under = 60` dans `pytest.ini`, couverture réelle 65% mesurée)*
- [x] **Les tests s'exécutent sans problème technique en environnement de test** (critère bloquant — vérifier que `pytest` se termine avec un exit code 0, sans erreur d'import ni d'exécution). *(49 tests, exit code 0 — voir aussi la correction du `default_timeout` de `test_ui_dashboard_qualite_shows_real_data`)*
- [x] Les sources sont versionnées et accessibles depuis un dépôt Git distant (DVC, GitLab, etc.).
- [x] La documentation couvre la procédure d'installation de l'environnement de test, les dépendances installées, la procédure d'exécution des tests et de calcul de la couverture. *(`--cov` intégré aux `addopts` de `pytest.ini`, rapporté aussi en CI)*
- [x] La documentation est communiquée dans un format qui respecte les recommandations d'accessibilité.
...
- [x] Les éventuels services (PaaS, SaaS, etc.) et prestataires ayant une démarche éco-responsable sont favorisés lors des choix techniques. *(voir `docs/COMPLEMENTS_COMPETENCES.md` § C15)*
- [x] Les flux de données impliqués dans l'application sont représentés par un diagramme de flux de données. *(diagramme Mermaid dans `docs/COMPLEMENTS_COMPETENCES.md` § C15)*
- [x] La preuve de concept est accessible et fonctionnelle en environnement de pré-production.
- [x] La conclusion à l'issue de la preuve de concept donne un avis précis permettant une prise de décision sur la poursuite du projet. *(avis GO documenté dans `docs/COMPLEMENTS_COMPETENCES.md` § C15)*

### C16 — Coordonner la réalisation technique (conduite agile / MLOps)

- [x] Les cycles, les étapes de chaque cycle, les rôles, les rituels et les outils de la méthode agile appliquée sont respectés dans sa mise en place et tout au long du projet. *(Kanban, cf. `docs/COMPLEMENTS_COMPETENCES.md` § C16)*
- [x] Les outils de pilotage (tableau kanban, burndown chart, backlog, etc.) sont disponibles dans les conditions prévues par la méthode appliquée. *(backlog rétrospectif reconstruit depuis l'historique Git + tableau GitHub Projects)*
- [x] Les objectifs et les modalités des rituels sont partagés à toutes les parties prenantes et rappelés si besoin. *(rituels documentés dans `docs/COMPLEMENTS_COMPETENCES.md` § C16)*
...
- [x] Les développements sont réalisés dans le respect des bonnes pratiques d'éco-conception d'une application (recommandations éco-index ou Green IT par exemple). *(analyse détaillée dans `docs/COMPLEMENTS_COMPETENCES.md` § C17 ; limite identifiée : pagination à ajouter sur `/api/measurements`)*
...
- [x] L'étape de livraison (pull request par exemple) est intégrée et exécutée une fois la ou les étapes de packaging validées. *(job `delivery` de `.github/workflows/ci.yml` : publication des images sur GHCR après succès du job `ci`, sur push vers `main`)*
...
### Points de vigilance historiques (résolus)

- ~~**C9 / C12** : `tests/test_pipeline.py::test_health_endpoint` échoue de façon reproductible selon l'ordre d'exécution des fixtures~~ — non reproduit sur la suite actuelle (49 tests, exit code 0). Un problème de même nature (timeout par défaut de `AppTest` trop court sous charge cumulée de la suite complète) a été identifié et corrigé sur `test_ui_dashboard_qualite_shows_real_data` (`default_timeout=15` dans `tests/test_ui_integration.py`).
- ~~**C13 / C18 / C19** : le pipeline n'était pas sous `.github/workflows/`~~ — corrigé : déplacé de `workflows/ci.yml` vers `.github/workflows/ci.yml` (seul emplacement détecté par GitHub Actions). Un job `delivery` (packaging + publication GHCR) a été ajouté, déclenché après succès du job `ci` sur push vers `main`.
- **`requirements.txt`** : toujours pas de versions figées (limite connue, documentée dans `docs/CI_CD.md`) — à traiter par un fichier de lock si la reproductibilité stricte des builds devient un enjeu.
